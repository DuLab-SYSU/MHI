...
def get_parser():
    description = 'Network-based separation of virus and disease.'
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument('-v', '--virus_host_ppi', type=str, required=True, help='Virus-host PPI')
    parser.add_argument('-d', '--disease_gene_file', type=str, required=True, help='Disease gene filename')
    parser.add_argument('-i', '--edge_list_file', type=str, required=True, help='Edge list filename')
    parser.add_argument('-o', '--disease_separation', type=str, required=True, help='Disease separation')
...
def get_disease_gene(disease_gene_file, network):
    disease_file = open(disease_gene_file)
    disease_gene = {}
    for row in itertools.islice(disease_file, 1, None):
        cols = row.strip().split('\t')
        disease = cols[0]
        if cols[1] in list(network.nodes()):
            disease_gene.setdefault(disease, []).append(cols[1])
    return disease_gene

def disease_separation_proximity(network, disease_gene, disease, nodes_to):
    """
    nodes_to here means virally-targeted genes
    """
    nodes_from = disease_gene[disease]
    d = calculate_observed_separation_proximity(network, nodes_from, nodes_to)
    return dict(zip([disease], [(d)]))
...
def run(args):
    ...
    #start = time.time()
    disease_gene = get_disease_gene(args.disease_gene_file, G)
    combs = list(itertools.product([G], [disease_gene], [i for i in disease_gene], [vtg]))
    with mp.Pool(mp.cpu_count()) as pool:
        res = pool.starmap(disease_separation_proximity, combs)
        pool.close()
        pool.join()
    #end = time.time()
    ...
    open(args.disease_separation, 'a').write("%s\t%s\n" % ('disease', 'separation'))
    open(args.expected_separation, 'a').write("%s\t%s\t%s\t%s\t%s\n" % ('disease', 'separation', 'z-score', 'separation_mean', 'separation_sd'))
    for i in res:
        for disease, separation in i.items():
            open(args.disease_separation, 'a').write("%s\t%f\n" % (disease, separation))
            if separation < 0:
                nodes_from = disease_gene[disease]
                m, s = calculate_random_separation_proximity(G, nodes_from, vtg)
                z = calculate_z_score(separation, m, s)
                open(args.expected_separation, 'a').write("%s\t%f\t%f\t%f\t%f\n" % (disease, separation, z, m, s))
