...
def get_parser():
    ...
    parser.add_argument('-p', '--DEPs', type=str, required=True, help='Differential expressed genes')
    parser.add_argument('-n', '--INs', type=str, required=True, help='Intermediated nodes')
...
def run(args):
    ...
    dep = get_dep_node(args.DEPs)
    dep = [i for i in dep if i in list(G.nodes())]
    print(len(dep))
    expanded_dep = []
    with open(args.edge_list_file) as f:
        for row in f:
            col = row.strip().split('\t')
            if (col[0] in dep) or (col[1] in dep):
                expanded_dep.extend([col[0], col[1]])
    expanded_dep = set(expanded_dep)

    start = time.time()
    drug_target = get_drug_target(args.drug_target_file, G)
    
    drug_exist = drug_e(args.out_dir)
    combs = list(itertools.product([G], [drug_target], [i for i in drug_target if i not in drug_exist], [expanded_dep], [args.out_dir]))
