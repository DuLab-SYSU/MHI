...
def get_parser():
    description = 'Network-based proximity of drug targets to side-effect proteins.'
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument('-p', '--se_gene_file', type=str, required=True, help='Side effect gene filename')
...
def get_se_gene(se_gene_file, network):
    se_file = open(se_gene_file)
    se_gene = {}
    for row in itertools.islice(se_file, 1, None):
        cols = row.strip().split('\t')
        se = cols[0]
        if cols[1] in list(network.nodes()):
            se_gene.setdefault(se, []).append(cols[1])
    return se_gene
...
def drug_e(dir):
    cmd = "ls %s" % (dir)
    drugs = os.popen(cmd)
    drugs = [i.replace('.txt\n', '') for i in drugs.readlines()]
    return drugs

def drug_se_proximity(network, drug_target, drug, se_gene, se, out_dir):
    nodes_from = set(drug_target[drug])
    nodes_to = set(se_gene[se])
    d, z, (m, s) = calculate_proximity(network, nodes_from, nodes_to)
    #open(out_dir + '/' + drug + ".txt", 'a').write("%s\t%s\t%s\t%s\t%s\t%s\n" % ('drug', 'side_effect', 'distance', 'z-score', 'distance_mean', 'distance_sd'))
    open(out_dir + '/' + drug + ".txt", 'a').write("%s\t%s\t%f\t%f\t%f\t%f\n" % (drug, se, d, z, m, s))
    return

def run(args):
    G = create_network_from_first_two_columns(args.edge_list_file, delim = '\t')

    se_gene = get_se_gene(args.se_gene_file, G)
    drug_list = get_drugs(args.candidate_drugs)
    drug_exist = drug_e(args.out_dir)
    drug_list2 = [i for i in drug_list if i not in drug_exist]

    start = time.time()
    drug_target = get_drug_target(args.drug_target_file, G)
    
    #open(args.drug_se_proximity, 'a').write("%s\t%s\t%s\t%s\t%s\t%s\n" % ('drug', 'side_effect', 'distance', 'z-score', 'distance_mean', 'distance_sd'))
    for drug in drug_list2:
        if drug in drug_target:
            open(args.out_dir + '/' + drug + ".txt", 'a').write("%s\t%s\t%s\t%s\t%s\t%s\n" % ('drug', 'side_effect', 'distance', 'z-score', 'distance_mean', 'distance_sd'))
            combs = list(itertools.product([G], [drug_target], [drug], [se_gene], [j for j in se_gene], [args.out_dir]))
    
            with mp.Pool(10) as pool:
                res = pool.starmap(drug_se_proximity, combs)
                pool.close()
                pool.join()        
