...
### for all FDA-approved drugs (2022-09-03)
dir.create('../result/DrugBank/drug_proximity', recursive = TRUE)

for (dire in list.dirs(path = "../intermediate/DrugBank/drug_proximity/", recursive = FALSE)) {
  ...
  write.table(drug_dist, 
              paste0('../result/DrugBank/drug_proximity/', virus, "_distance.txt"),
              quote = FALSE, sep = "\t", row.names = FALSE)
}

## Number of drugs prioritized across poxviruses and number of viruses for each drugs (2022-09-21)
drug_proximity <- data.frame()
drug_proximity_signif <- data.frame()
for (f in list.files(path = "../data/p-hipster/")){
  v <- sub(".txt", "", f)
  drug_dist <- read.table(paste0("../result/DrugBank/drug_proximity/", v, "_distance.txt"),
                          header = TRUE, sep = '\t', quote = '')
  drug_dist <- drug_dist %>% mutate(Virus = v)
  drug_dist2 <- drug_dist %>% filter(z.score < -1.6)
...
ggsave(p_virus_drug_num,
       filename = "../result/Fig_S3AB.pdf",
       width = 15,
       height = 7,
       units = c("cm"))


## heatmap highlighting network proximity value for 12 poxviruses (2022-09-12)
...
#write_xlsx(list(drug_proximity = drug_proximity, drug_proximity_zscore = drug_proximity_z),
#           '../result/drug_proximity.xlsx')
MPXV_drugs <- drug_proximity_z_long %>% 
  filter(Abbreviation == 'MPXV') %>%
  select(drug)

write.table(MPXV_drugs, 
            '../result/MPXV_277candidate_drugs.txt',
            quote = FALSE, 
            sep = "\t", 
            row.names = FALSE,
            col.names = FALSE)

## Pearson correlation coefficients of the proximities for each viral pair (2022-09-03)
