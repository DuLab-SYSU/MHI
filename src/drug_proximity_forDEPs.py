...
def get_parser():
    description = 'Network-based separation of drug target and DEPs.'
...
def get_dep_node(dep_file, reg = 'all'):
    nodes = []
...
def run(args):
    ...
    dep = get_dep_node(args.DEPs, reg = args.regulation)
    dep = [i for i in dep if i in list(G.nodes())]

    start = time.time()
    drug_target = get_drug_target(args.drug_target_file, G)
    combs = list(itertools.product([G], [drug_target], [i for i in drug_target], [dep], [args.out_dir]))
