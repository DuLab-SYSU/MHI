...
def get_parser():
    ...
    parser.add_argument('-o', '--drug_se_proximity', type=str, required=True, help='Statistic of proximity')
    return parser
...
def get_drugs(drug_file):
    ...
    drug_list = []
    for row in drug_file:
        drug = row.strip()
        drug_list.append(drug)
    return drug_list

def drug_se_proximity(network, drug_target, drug, se_gene, se, out_file):
...
def run(args):
    ...
    for drug in drug_list:
        combs = list(itertools.product([G], [drug_target], [drug], [se_gene], [j for j in se_gene], [args.drug_se_proximity]))
